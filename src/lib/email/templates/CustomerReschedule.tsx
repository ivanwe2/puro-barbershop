...
interface CustomerRescheduleProps {
  name: string;
  date: string;
  time: string;
  serviceName: string;
...
export function CustomerReschedule({
  ...
  phone,
  locale = "bg",
}: CustomerRescheduleProps) {
  const s = emailStrings(locale);
  return (
    <Html>
      <Head />
      <Preview>{s.subjects.reschedule}</Preview>
      <Body style={body}>
        <Container style={container}>
          <Heading style={heading}>{s.reschedule.title}</Heading>
          <Text style={text}>{s.greeting(name)}</Text>
          <Text style={text}>{s.reschedule.intro}</Text>
